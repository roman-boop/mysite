...
// BingX perpetual swap symbols (matching get_all_tickers format)
...
// Matches BingxClient._public_request + get_klines
async function fetchBingXKlines(symbol: string, interval = '4h', limit = 180) {
  const params = new URLSearchParams({
    symbol,
    interval,
    limit: String(limit),
  });
  const url = `${BINGX_BASE}/openApi/swap/v3/quote/klines?${params.toString()}`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[Market/Analysis] fetchBingXKlines HTTP ${res.status} for ${symbol}: ${body}`);
      return null;
    }
    const json = await res.json();
    if (!json || json.code !== 0) {
      console.error(`[Market/Analysis] fetchBingXKlines BingX error code=${json?.code} msg=${json?.msg} for ${symbol}`);
      return null;
    }
    const rows: any[] = json.data ?? [];
    if (!rows.length) {
      console.error(`[Market/Analysis] fetchBingXKlines empty data for ${symbol}`);
      return null;
    }
    ...
    return rows
      ...
      .sort((a, b) => 0); // already sorted by BingX
  } catch (err: any) {
    clearTimeout(timeout);
    if (err?.name === 'AbortError') {
      console.error(`[Market/Analysis] fetchBingXKlines timeout for ${symbol}`);
    } else {
      console.error(`[Market/Analysis] fetchBingXKlines error for ${symbol}:`, err?.message ?? err);
    }
    return null;
  }
}
...
export async function GET() {
  try {
    console.log('[Market/Analysis] Starting BingX market analysis for', CRYPTO_SYMBOLS.length, 'symbols');

    const results = await Promise.allSettled(
      CRYPTO_SYMBOLS.map((sym) => fetchBingXKlines(sym, '4h', CONFIG.lookback_limit))
    );

    const successCount = results.filter((r) => r.status === 'fulfilled' && r.value !== null).length;
    console.log(`[Market/Analysis] Fetched ${successCount}/${CRYPTO_SYMBOLS.length} symbols from BingX`);

    if (successCount === 0) {
      console.error('[Market/Analysis] All BingX klines fetches failed — BingX API may be unreachable');
      return NextResponse.json(
        { error: 'Failed to fetch market data — BingX API unreachable from server' },
        { status: 500 }
      );
    }
    ...
    if (!agg) {
      console.error('[Market/Analysis] aggregate() returned null — insufficient candle data');
      return NextResponse.json(
